...
bool RsuArmComm::openComm()
{
    if ( !pSerialPort->open() )
    {
        qWarning("Cannot open serial port %s. Exiting..", qPrintable(pSerialPort->portName()));
        return false;
    }
...
{
    return pSerialPort->setPortSettings(portSettings);
}
...
{
    QByteArray tempdata=pSerialPort->read(pSerialPort->bytesAvailable());
    DebugMessage("接收到的报文"+QString(tempdata.toHex()));
    //先将接收到的报文加入队列中
    m_RecivedRawData.append(tempdata);
    DebugMessage("队列中报文"+QString(m_RecivedRawData.toHex()));
    //清空接收帧数据的内存
    m_RecivedFrameData.clear();
    int step=0;
    int startIndex=0;
...
void RsuArmComm::InputTrans(QByteArray &message)
{
    ...
    {
        if(message.at(i)==(char)0xfe && i!=message.length()-1)
        {
