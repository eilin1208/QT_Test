...
#define MAX_EVENTID_LEN             64      //EventID lenth
#define MAX_IPV6_LEN  			    64      //The maximum length of the IPv6 address
...
/*****************************video start****************************/
#define	NET_DVR_MATRIX_WALL_SET		            9001  //set video wall config
#define	NET_DVR_MATRIX_WALL_GET	                9002  //get video wall config
#define	NET_DVR_WALLWIN_GET			            9003  //get video wall window config 
#define	NET_DVR_WALLWIN_SET			            9004  //set video wall window config 
...
#define	NET_DVR_WALLOUTPUT_GET		            9011  //get output config
#define	NET_DVR_WALLOUTPUT_SET		            9012  //set output config
...
#define NET_DVR_SET_OSDSHOWCFG              		27000   		//设置OSD显示


/**********************************Sicep 72SH OEM start************************************************/
#define NET_DVR_SET_CONTROL_CODE            0x27003    //Set Control Code
/**********************************Sicep 72SH OEM end************************************************/
...
#define  DS_65XXD_B10_CLASS			323		//B10 65XXD decoder 
...
#define  DS_6416HFH_B20S            401             //6416HFH_B20S
#define  DS_6416HFH_B20_RM          402             //6416HFH_B20_RM  
#define  DS_6564D_B20D              403             //6564D_B20D
#define  DS_6564D_B20H              404             //6564D_B20H
#define  DS_6564D_B20V              405				//6564D_B20V
#define  DS_B20_MSU_T               406				//B20_MSU_T
...
#define	MINOR_GET_SCREEN_MODE_CFG		0x289		//get screen mode 
#define	MINOR_SET_SCREEN_MODE_CFG		0x290		//set screen mode
...
typedef struct 
{
    ...
    4-2CIF(P704×288), 6-QVGA(320*240), 7-QQVGA(160*120), 13-576*576, 16-VGA(640*480), 17-UXGA(1600*1200), 
    18-SVGA(800*600), 19-HD720p(1280*720), 20-XVGA(1280x960), 21-HD900p(1600x900), 22-SXGAp(1360*1024), 23-1536*1536,24-1920*1920,
...
typedef  struct tagNET_DVR_HKDDNS_STREAM
{
    BYTE    byEnable;   //Is enable
    BYTE    byRes[3];
    BYTE    byDDNSDomain[64];    // hkDDNS domain
    WORD	wPort;                   //IPServer port
...
typedef union tagNET_DVR_GET_STREAM_UNION
{
	...
	NET_DVR_HKDDNS_STREAM struHkDDNSStream;   //get stream through HkDDNS
...
typedef struct
{
    ...
    DWORD dwHdStatus;      /*Disk status (Read only)  0- Normal,  1- Raw,  2- Error,  3- SMART State,  4- Unmatched,  5- Sleep*/
	                       ...
	                       /*14 - the local hard disk does not exist 15 - (network hard drive being deleted)*/
...
typedef struct
{
	...
	BYTE byMultiStream;//Support multi-stream, epresent by bit, 0 - not support 1 - support bit0-stream bit1-stream, and so on
	BYTE byMultiStreamProto;//support rtsp protocols, represent by bit, 0-not support ;1- support; bit0-code stream 3 ;bit1-stream, and so on
    BYTE byRes2[4];					//Reserved	
	BYTE byControlCode[CONTROL_CODE_LEN ];   //Control Code
...
typedef struct tagNET_DVR_PREVIEWINFO{
	LONG	lChannel; 		// Channel no.
	DWORD	dwStreamType;	// Stream type 0-main stream，1-sub stream，2-third stream，3-forth stream, and so on
	...
	BOOL bBlocked;          //If data stream requesting process is blocked or not: 0-no, 1-yes  
	                        //if true, the SDK Connect failure return util 5s timeout  , not suitable for polling to preview.
	BOOL bPassbackRecord; //0- not enable  ,1 enable
...
{   
    ...
    BYTE byRes2; 
	BYTE byControlCode[CONTROL_CODE_LEN];
...
{
    BYTE  byAudioEncType;    //AudioTalk Code Type 0-OggVorbis; 1-G711_U; 2-G711_A;6-G726
...
typedef struct tagNET_VCA_AUDIO_ABNORMAL
{
	...
	BYTE bySensitivity;    /* Sensitivity range: [1, 5] */
    BYTE byAudioMode;    //Audio mode,0-enable sensitivity detect,1-enable decibel threshold   
    BYTE byRes[56];            
...
typedef enum _MAIN_EVENT_TYPE_
{
	...
	EVENT_AUX_ALARM = 4,		//Auxiliary alarm add by wxm 12-12-17 for IPC V5.0
...
typedef struct tagNET_DVR_SEARCH_EVENT_PARAM
{
    ...
    union	
    {
		...
		struct								//Auxiliary alarm
		{
			BYTE byChanNo[MAX_CHANNUM_V30];	//Trigger auxiliary alarm channel
			BYTE byRes[SEARCH_EVENT_INFO_LEN - MAX_CHANNUM_V30];
		}struAuxAlarmParam;
    }uSeniorParam; 
...
{
    BYTE byWhiteBalanceMode;  /*0-manual white balance;  1-ATW1(small range) ;  2- ATW2  (2200K- 15000K) ; 3-Hold Current,4-Fixed Incandescent,5-Fixed Warm-light,6-Fixed Natural-light,7-Fixed Fluorecent*/
...
typedef struct tagNET_DVR_VEHICLE_INFO_
{
    ...
    BYTE  byIllegalType;            //0- normal; 1- low speed 2- High speed 3- traverse travelling,4-run red light,5-drive line,6-direction, 7 - intersection stranded, 8 - machines accounting for non-9 - illegal lane change
	BYTE  byRes2[3]; //Reserved
...
typedef struct
{
	...
	//bySupport1 & 0x20, support rtsp over http
	WORD wDevType;//Device type
	BYTE  byDevTypeName[24];//Device model name
...
typedef struct tagNET_DVR_ALARMIN_PARAM
{
	...
	BYTE	byModuleType;	//Module type，1-local zone，2-single zone，3-double zone，4-8 zone，5-8 sensor zone
...
typedef struct tagNET_DVR_ALARMOUT_PARAM
{
	...
	BYTE    byAssociateAlarmIn[MAX_ALARMHOST_ALARMIN_NUM];   //Alarm input channel followed by siren(multi alarm inputs trigger one siren output)
	                            //Subscript 0 means alarm input 1, 0- not follow, 1- follow
	BYTE	byModuleType;	//trigger type，1-local trigger， 2-4-way trigger ，3 8-way trigger
...
typedef struct tagNET_DVR_ALARMHOST_ABILITY
{
	...
	DWORD	dwFaultType;			//fault type，0- not support, 1-support, bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal
...
{
	//bit0 表示，be associated by the first public subsystem
...
typedef struct tagNET_DVR_PRINTER_ENABLE
{
	...
	BYTE	byPrintTime;		
	BYTE	byRes1[2];
	DWORD	dwAlarmInfo;			//alarm information，bit0-Zone alarm，bit1-urge alarm ，bit-2 hostage alarm
	DWORD	dwDeviceInfo;			//device information，bit0-AC Current、bit1-battery、bit2-phone line、bit3-test report、bit4-demolish、bit5-keyboard off-line、bit6-SIM、bit7-GPRS/3G、bit8-network、bit9-MBUS Abnormal
	DWORD	dwOperateInfo;			//operate information，bit0-arm、bit1-disarm、bit2-clear alarm、bit3-bypass、bit4-enter program、bit5-exit program、bit6-restore
	BYTE 	byRes2[256];	
...
typedef struct tagNET_DVR_SCREENCONTROL_ABILITY
{
	...
	BYTE	bySupportVCSUserCfg;	//support user config
	BYTE	byRes[53];
...
{
	DWORD	dwSize;
	DWORD	dwOverallEventTriggerAlarmoutOn;	//overall event cause alarmout on ，bit0-AC outage，bit1-low voltage of battery，bit2-telephone off_line，bit3-network abnormal，bit4-Wireless network abnormal
	DWORD	dwOverallEventTriggerAlarmoutOff;	//overall event cause alarmout off，bit0-AC outage，bit1-low voltage of battery，bit2-telephone off_line，bit3-network abnormal，bit4-Wireless network abnormal
	DWORD	dwSubSystemEventTriggerAlarmoutOn[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //subsystem event cause alarmout on,dwSubSystemEvent[0] is subsystem 1，dwSubSystemEvent[2] is subsystem 1，every bit is a subsystem event，bit0-enter delay time，bit1-exit delay time，bit2-arm，bit3-disarm，bit4-alarm， bit5-clear alarm
	DWORD	dwSubSystemEventTriggerAlarmoutOff[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //subsystem event cause alarmout off,wSubSystemEvent[0] is subsystem 1，dwSubSystemEvent[2] is subsystem 2，every bit is a subsystem event，bit0-enter delay time，bit1-exit delay time，bit2-arm，bit3-disarm，bit4-alarm， bit5-clear alarm
	BYTE	byRes[128];
...
{
	DWORD 	dwSize;
	DWORD	dwCheckFault;	//Check fault，bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal
	DWORD	dwOverallFaultJointLED;		//fault cause overall keyboard led turn on, bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal
	DWORD	dwOverallFaultJointSound;	//fault cause overall keyboard sound turn on, bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal
	DWORD	dwSubSystemFaultJointLED[MAX_ALARMHOST_SUBSYSTEM]; /*fault cause subsystem keyboard led turn on，dwSubSystemFaultJointLED[0] associate to first subsystem。every bit means one fault of subsystem in dwSubSystemFaultJointLED[0]，bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal*/
	DWORD	dwSubSystemFaultJointSound[MAX_ALARMHOST_SUBSYSTEM];/*fault cause subsystem keyboard sound turn on, dwSubSystemFaultJointLED[0]associate to first subsystem。every bit means one fault of subsystem in dwSubSystemFaultJointSound，bit0-AC outage，bit1-low voltage of battery，bit2-device prevent disassemble，bit3-telephone off_line，bit4-main keyboard off_line，bit5-network abnormal，bit6-GPRS abnormal，bit7-MBUSabnormal，bit8-hard disk abnormal*/
	BYTE	byRes[64];
...
typedef struct tagNET_DVR_CID_ALARM
{
	...
	BYTE	byUserType;						//User type，0-net user 1-keyboard user
...
typedef struct tagNET_ITC_POST_RS485_PARAM
{
	...
	BYTE byTriggerSpareMode; //Trigger standby mode, 0 - default, 1 - bayonet virtual coil mode
...
typedef struct tagNET_DVR_STRUCTHEAD
{
    ...
    BYTE	byVersion ;	    /*High and low 4 respectively represent the high and low version, 
							subsequent editions and length to extend the length of the different versions of limit制*/
    BYTE	byRes;
...
#define NET_DVR_GET_HTTPS_CFG		        6134		//Get HTTPS information
#define NET_DVR_SET_HTTPS_CFG		        6135		//Set HTTPS information
...
typedef struct tagNET_DVR_FTP_CFG_V40
{
	...
	BYTE 				byTopDirMode;	    /* Top directory，0x1 = use device name, 0x2 = use device No,0x3 = use device ip，
											   0x4=use Monitoring points ,0x5=Time (Date)  0x6-Using a custom*/
	BYTE 				bySubDirMode;       /*Sub directory，0x1=use channel name,0x2=use channel number
											   0x3=use time (Date),0x4=use Lane number 0x5-Using a custom*/
...
{
	DOWNLOAD_CERT = 0,
	DOWNLOAD_ALARM_RECORD = 1 //Download alarm recording
...
{
	BYTE	bySerPortNum;   // Serial port
	BYTE    byMatrixSerPortType;//0 --- Matirx protocol 1 --- keyboard protocol
	BYTE	byRes1[2];
	NET_DVR_SINGLE_RS232 struRS232;	//232 serial port parameter
	BYTE	byRes2[200];      
...
typedef struct tagNET_DVR_VCS_USER_INFO
{
	...
	//bit0-get base parameter
	//bit1-set base parameter
	//bit2-manage screen
	//bit3-manage window 
	//bit4-manage signal source
	//bit5-manage layout
	//bit6-manage plan
	//bit7-manage picture
	//bit8-manage OSD
	...
	//bit11-upgrade
	//bit12-resume
	//bit13-input and output config file
	//bit14-search log
	//bit15-restart
...
typedef struct tagNET_DVR_SAFETYCABIN_PERSON_SIGNAL_CFG
{
	...
	BYTE  byDevUseTimeout; //cabin use timeout,unit:min,range:0-30,default:15min,upload alarm when timeout
	BYTE  byRes1;
...
typedef struct tagNET_DVR_CAMERA_SETUPCFG
{
	...
	BYTE byLensType; //The focal length of the lens type，0-8mm,1-12mm,2-16mm,3-25mm,4-35mm,5-50mm
    BYTE byRes[61]; 
...
typedef struct  tagNET_DVR_CONTROL_CODE_INFO
{
	DWORD   dwSize ;   
	BYTE    byControlCode[CONTROL_CODE_LEN]; /*Control Code*/
	BYTE    byRes[16] ;       
}NET_DVR_CONTROL_CODE_INFO, *LPNET_DVR_CONTROL_CODE_INFO;



/********************************北分瑞华赢定制*********************************/
#define MAX_OSDSHOWCHAR_NUM 512


typedef struct tagNET_DVR_OSDSHOWCFG
{	
    DWORD  dwSize;
    WORD   wXCoordinate;   /*OSD在屏幕左上角位置x*/
    WORD   wYCoordinate;   /*OSD在屏幕左上角位置y*/
    DWORD  dwCharCnt;     /*字符的个数*/
    char   sOSDChar[MAX_OSDSHOWCHAR_NUM];       /*OSD字符内容*/
    BYTE   byRes[32];
}NET_DVR_OSDSHOWCFG, *LPNET_DVR_OSDSHOWCFG;
/********************************北分瑞华赢定制*********************************/
