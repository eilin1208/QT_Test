...
void MyTelnetWidget::processIncomingData(const QString &text)
{
    ...
    qDebug()<<"远程来的数据"<<reformatedText;
    QRegExp exp("\\[.+m");
    exp.setMinimal(true);
    reformatedText = reformatedText.replace(exp, "");
    emit termMessage(reformatedText);
...
void MyTelnetWidget::keyPressEvent(QKeyEvent * ke)
{
    ...
    {
        sforClear.append(ke->key());
        if(sforClear.length()>=10)
            sforClear.clear();
        switch (ke->key())
        {
        case Qt::Key_Return:
        ...
        {
            t->sendData("\r\n");
            if(sforClear.contains("clear",Qt::CaseInsensitive))
            {
                this->clear();
                sforClear.clear();
            }
