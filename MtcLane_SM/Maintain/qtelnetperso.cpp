...
int QTelnetPerso::parsePlaintext(const QByteArray &data)
{
    ...
    const QString text = QString::fromLocal8Bit(data.constData(),data.length());
    if (!text.isEmpty())
    {
//        QString tempString;
//        QRegExp temp("\[.+m");
//        tempString.replace(temp,tr(""));
        emit message(text);
    }
...
QString QTelnetPerso::FilterColor(const QString &message)
{
        ...
        if(flag)
        {
            for(int j=0;j!=temp.length();j++)
            {
                if(temp.at(j)=='m')
                {
                    ...
                    if(end-begin<=6)
                        temp=temp.mid(0,begin-0)+temp.mid(end);
                    begin=0;
                    end=0;
                    flag=false;
                }
