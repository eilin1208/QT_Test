...
#include "ui_testprinter.h"
#include "CommonModule/LaneType.h"
#include "CommonModule/laneutils_new.h"
TestPrinter::TestPrinter(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TestPrinter)
{
    ui->setupUi(this);
    //打开设备
...
void TestPrinter::on_pushButton_openDev_clicked()
{
...
void TestPrinter::on_pushButton_CloseDev_clicked()
{
...
void TestPrinter::on_pushButton_printer_clicked()
{
    SetNoteContent();
...
void TestPrinter::SetNoteContent()
{
    ...
    //收费类型
    if(ui->comboBox_feetype->currentText().toInt() == 0)
    {
    ...
    else if(ui->comboBox_feetype->currentText().toInt() == 1)
    {
