...
QUrlInfo FtpDirModel::getUrlInfo(const QModelIndex &index)
{
    FtpEntry* entry = (FtpEntry*)index.internalPointer();
    return entry->urlInfo();
}

QString FtpDirModel::getDir(const QModelIndex &index)
{
    QStringList list;
    QModelIndex tmpIndex = index;
    while(tmpIndex.isValid())
    {
        list.append(FtpDirModel::getUrlInfo(tmpIndex).name() + "/");
