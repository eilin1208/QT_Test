...
namespace Log4Qt
{
	...
	class FileAppender : public WriterAppender
	{
	     ...
	     */
        Q_PROPERTY(bool append READ appendFile WRITE setAppendFile)
        ...
        /*!
         * The property holds the path of the file.
         *
         * \sa filePath(), setFilePath()
         */
	...
	public:
        ...
        void setFilePath(const QString &rFilePath);
        void setSuffix(const QString &rSuffix);
        void setFullFileName(const QString &rFullFile);
    ...
    inline QString FileAppender::fullFileName() const
    {
        QMutexLocker locker(&mObjectGuard);
        return mFullFileName;
    }

    inline QString FileAppender::filePath() const
    {   QMutexLocker locker(&mObjectGuard);
        return mFilePath;   }

    inline QString FileAppender::suffix() const
    {   QMutexLocker locker(&mObjectGuard);
        return mSuffix;   }
	...
	{   QMutexLocker locker(&mObjectGuard);
        mFullFileName = mFilePath+rFileName + mSuffix;
	    mFileName = rFileName;   }

    inline void FileAppender::setFilePath(const QString &rFilePath)
    {   QMutexLocker locker(&mObjectGuard);
        mFullFileName = rFilePath+mFileName + mSuffix;
        mFilePath = rFilePath;   }

    inline void FileAppender::setSuffix(const QString &rSuffix)
    {   QMutexLocker locker(&mObjectGuard);
        mFullFileName = mFilePath+mFileName + rSuffix;
        mSuffix = rSuffix;   }

    inline void FileAppender::setFullFileName(const QString &rFullFile)
    {   QMutexLocker locker(&mObjectGuard);
        mFullFileName = rFullFile;   }
