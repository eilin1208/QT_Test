...
namespace Log4Qt
{
	...
	*************************************************************************/
	...
	void RollingFileAppender::rollOver()
	{
        ...
        QString target_file_name = filePath() + fileName() +  QLatin1Char('.')
                            + tr("%1").arg(mMaxBackupIndex, 2, 10, QLatin1Char('0')) + suffix();
	    QFile f;
        f.setFileName(target_file_name);
	    ...
	    {
            target_file_name = filePath() + fileName() +  QLatin1Char('.')
                                        + tr("%1").arg(i, 2, 10, QLatin1Char('0')) + suffix();
            f.setFileName(target_file_name);
	        if (f.exists())
	        {
                target_file_name = filePath() + fileName() +  QLatin1Char('.')
                        + tr("%1").arg(i+1, 2, 10, QLatin1Char('0')) + suffix();
        ...
        target_file_name = filePath() + fileName() + QLatin1String(".01") + suffix();
	    if (!renameFile(f, target_file_name))
	        return;
