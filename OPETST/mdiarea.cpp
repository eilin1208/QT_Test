...
{
    MdiWindow* mdiWin = GetMdiChid(mdiChild->GetMdiChildID());
    if (mdiWin)
    {
        setActiveSubWindow(mdiWin);
        return mdiWin;
    }
    mdiWin = new MdiWindow(mdiChild, this);
...
MdiWindow* MdiArea::GetMdiChid(int nID) const
{
    QList<MdiWindow*> list;
    foreach (MdiWindow *window, getWindows())
    {
        if (window->getMdiChild()->GetMdiChildID() == nID)
            return window;
    }
    return nullptr;
}
